...
export default function CreatorWorkspace({
  ...
  const handleFileUploadAndSubmit = async () => {
    ...
    try {
      ...
      // 3. Create the submission record in Convex, including the optional
      // link to the live reel/story so the brand can open it directly.
      await createSubmission({
        campaignId: activeWorkingCampaign.id as any,
        fileId: storageId,
        contentUrl: contentLink.trim() || undefined,
      });
    } catch (err: any) {
                    ...
                    <div className="flex flex-col gap-6 pt-4">
                      <div>
                        ...
                        <p className="text-sm text-zinc-400 mt-1 max-w-2xl">
                          Upload the actual reel, story, or post you made for this campaign — a screen recording or export works. The brand will preview it before releasing your payout.
                        </p>
                      </div>
                      ...
                      >
                        ...
                        <div className="flex flex-col gap-1 text-center">
                          <span className="text-base font-bold text-zinc-200">
                            {selectedFile && expandedCampaignId === camp.id ? selectedFile.name : 'Click or drag your deliverable to upload'}
                          </span>
                          <span className="text-xs font-mono text-zinc-500 uppercase tracking-wider mt-1">Photo or video, up to 10MB</span>
                        </div>
                      </div>

                      {/* Optional link to the live post/reel/story */}
                      <div className="flex flex-col gap-2">
                        ...
                        <span className="text-xs text-zinc-500">
                          Lets the brand open the actual reel/story directly, in addition to your upload.
                        </span>
                      </div>
                        ...
                        >
                          {isUploadingThis ? (
                            ...
                            <>
                              ...
                              <span>Submit Deliverable Securely</span>
                            </>
                          )}
                        </button>
                      )}
                    </div>
                  ) : submission.status === 'uploaded' || submission.status === 'verifying' ? (
                      ...
                      <div className="flex-1 flex flex-col gap-6">
                        <div className="flex flex-col gap-2">
                          <h4 className="text-xl font-display font-black text-white flex items-center gap-3">
                            ...
                            {submission.status === 'verifying' ? 'Brand Is Reviewing Your Deliverable' : 'Deliverable Submitted Successfully'}
                          </h4>
                          <p className="text-sm text-zinc-400 leading-relaxed max-w-xl">
                            <span className="text-white font-bold">{camp.brandName}</span> previews your uploaded content{submission.contentUrl ? ' and live link' : ''} directly —
                            payout releases instantly via smart escrow once they approve it.
                          </p>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4 bg-zinc-900/50 rounded-2xl p-4 border border-zinc-800">
                          <div className="flex flex-col gap-1">
                            ...
                            <span className="text-sm text-indigo-400 font-bold flex items-center gap-2">
                              ...
                              {submission.status === 'verifying' ? 'In Review' : 'Awaiting Review'}
                            </span>
                          </div>
                        ...
                        <span className="text-xs font-mono text-zinc-500 opacity-80">
                          * No location or EXIF check is used — the brand reviews the actual content manually.
                        </span>
                      </div>
                    </div>
                  ) : submission.status === 'rejected' ? (
                    <div className="bg-rose-500/10 border border-rose-500/20 rounded-3xl p-8 flex flex-col items-center text-center gap-5 animate-fade-in relative overflow-hidden">
                      <div className="absolute top-0 right-0 w-full h-1 bg-gradient-to-r from-transparent via-rose-500 to-transparent opacity-50" />
                      <div className="w-16 h-16 rounded-full bg-rose-500/20 flex items-center justify-center text-rose-400 shadow-[0_0_30px_-5px_rgba(244,63,94,0.4)]">
                        <AlertTriangle className="w-8 h-8" />
                      </div>
                      <div className="flex flex-col gap-2 max-w-md">
                        <h4 className="text-xl font-display font-black text-rose-400">Deliverable Was Not Approved</h4>
                        <p className="text-sm text-zinc-400 leading-relaxed">
                          <span className="text-white font-bold">{camp.brandName}</span> didn't approve this submission
                          {submission.rejectionReason ? `: "${submission.rejectionReason}"` : '.'} You can upload a new deliverable to try again.
                        </p>
                      </div>
                    </div>
                  ) : (
